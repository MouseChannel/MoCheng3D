...
SwapChain::SwapChain() {
  ...
  auto graphic_queue_index = Get_Context_Singleton()
                                 .Get_Device()
                                 ->queue_family_indices.graphic_queue.value();

  auto present_queue_index = Get_Context_Singleton()
                                 .Get_Device()
                                 ->queue_family_indices.present_queue.value();

  auto share_mode = graphic_queue_index == present_queue_index
                        ? vk::SharingMode::eExclusive
                        : vk::SharingMode::eConcurrent;

  std::set<uint32_t> queue_family_index{graphic_queue_index,
                                        present_queue_index};

  std::vector<uint32_t> queue_family_index_v;
  queue_family_index_v.assign(queue_family_index.begin(),
                              queue_family_index.end());
  ...
  m_handle =
      Get_Context_Singleton().Get_Device()->Get_handle().createSwapchainKHR(
          createInfo);
}
SwapChain::~SwapChain() {}
...
SwapChain::Query_surface_Extent(const vk::SurfaceCapabilitiesKHR &capability,
  ...
  if (capability.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
    ...
    extent.width = std::clamp(extent.width, capability.minImageExtent.width,
                              capability.maxImageExtent.width);
    extent.height = std::clamp(extent.height, capability.minImageExtent.height,
...
vk::SurfaceFormatKHR SwapChain::Query_surface_Format() {
  auto physical_device =
      Get_Context_Singleton().Get_Device()->Get_Physical_device();
  auto surface = Get_Context_Singleton().Get_Window()->GetSurface();
