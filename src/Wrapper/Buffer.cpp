...
void Buffer::Update(void *data, size_t size) {
  ...
  auto ptr = Get_Context_Singleton().Get_Device()->Get_handle().mapMemory(
      memory, 0, size);
  std::memcpy(ptr, data, size);
  Get_Context_Singleton().Get_Device()->Get_handle().unmapMemory(memory);
}

Buffer::Ptr Buffer::CreateDeviceBuffer(void *data, size_t size,
                                       vk::BufferUsageFlags usage) {
  auto command_buffer = CommandBuffer::Create();
  auto host_buffer =
      Buffer::Create(size, vk::BufferUsageFlagBits::eTransferSrc,
  ...
  auto device_buffer =
      Buffer::Create(size, vk::BufferUsageFlagBits::eTransferDst | usage,
  ...
  auto command_queue =
      Context::Get_Singleton().Get_Device()->Get_Graphic_queue();
